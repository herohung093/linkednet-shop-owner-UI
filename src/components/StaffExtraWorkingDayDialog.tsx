...
import { Trash2, Plus, X, CalendarPlus } from "lucide-react";
import { axiosWithToken } from "../utils/axios";
import ActionResultDialog from "./dialogs/ActionResultDialog";

interface StaffExtraWorkingDayDialogProps {
  staffId: number;
  staffName: string;
  open: boolean;
  onClose: () => void;
...
const StaffExtraWorkingDayDialog: React.FC<StaffExtraWorkingDayDialogProps> = ({
  staffId,
  staffName,
  open,
  onClose,
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching staff extra working days:", error);
    } finally {
      setLoading(false);
    }
  }, [staffId]);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setActionResultMessage("Extra shift added");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setActionResultMessage("Extra shift removed");
      setActionResultType("success");
      fetchRecords();
    } catch (error) {
            ...
            <Box sx={{ textAlign: "center", py: 5, px: 3 }}>
              <Box sx={{ display: "flex", justifyContent: "center", mb: 1.5 }}>
                <Box sx={{ p: 2, borderRadius: "50%", backgroundColor: "#f3f4f6" }}>
                  <CalendarPlus size={18} color="#9ca3af" />
                </Box>
              </Box>
              ...
              <Typography variant="caption" sx={{ color: "text.secondary" }}>
                {showAddForm ? "Fill in the form above to add one" : "Tap Add to mark extra shifts"}
              </Typography>
            </Box>
          )}
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2.5 }}>
                      ...
                      <Box
                        sx={{
                          ...
                          p: "4px 6px",
                          borderRadius: 1,
                          backgroundColor: past ? "#fef3c7" : "#f0fdf4",
                        }}
                      >
                        <Typography
                          variant="caption"
...
export default StaffExtraWorkingDayDialog;
