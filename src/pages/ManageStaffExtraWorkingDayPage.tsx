...
import { CalendarPlus, AlertCircle, Trash2, Plus, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";
import ActionResultDialog from "../components/dialogs/ActionResultDialog";

interface ExtraWorkingDayFormData {
  staffId: number;
  date: string;
  note: string;
}

const ManageStaffExtraWorkingDayPage: React.FC = () => {
  const [staffList, setStaffList] = useState<Staff[]>([]);
  const [selectedStaffId, setSelectedStaffId] = useState<number | "" | "all">("");
  const [records, setRecords] = useState<StaffExtraWorkingDay[]>([]);
  const [loading, setLoading] = useState(false);
  const [openDialog, setOpenDialog] = useState(false);
  const [formData, setFormData] = useState<ExtraWorkingDayFormData | null>(null);
  ...
  const fetchRecords = async (staffId: number) => {
    setLoading(true);
    try {
      const response = await axiosWithToken.get<StaffExtraWorkingDay[]>(
        `/staffExtraWorkingDay/${staffId}`
      );
      setRecords(
      ...
      console.error("Error fetching extra working day records:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchAllRecords = async () => {
    ...
    try {
      ...
      console.error("Error fetching all extra working day records:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosWithToken.delete(`/staffExtraWorkingDay/${recordToDelete}`);
      setActionResultMessage("Extra shift removed successfully");
      setActionResultType("success");
      refetchCurrent();
    } catch (error) {
      setActionResultMessage("Failed to remove extra shift");
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axiosWithToken.post("/staffExtraWorkingDay/", {
        staff: { id: formData.staffId },
        date: formData.date,
        note: formData.note,
      });
      setActionResultMessage("Extra shift added successfully");
      setActionResultType("success");
      refetchCurrent();
      setOpenDialog(false);
    } catch (error) {
      setActionResultMessage("Failed to add extra shift");
      setActionResultType("failure");
    } finally {
      setActionResultOpen(true);
    }
      ...
      <div className="bg-white border border-gray-200 rounded-lg" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        ...
        <div className="px-6 pt-6 pb-5 border-b border-gray-100">
          <div className="flex items-start gap-3 mb-5">
            <div className="mt-0.5 p-1.5 rounded-md bg-green-50">
              <CalendarPlus className="w-5 h-5 text-green-600" />
            </div>
            <div>
              <h1 className="text-base font-semibold text-gray-900">Staff Extra Shifts</h1>
              <p className="text-[13px] text-gray-500 mt-0.5">
                Manage one-off shifts outside regular working days
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
            <div className="relative flex-1 sm:max-w-sm">
              <select
                ...
                className="w-full appearance-none bg-white border border-gray-200 rounded-md px-3 py-2 pr-8 text-[13px] text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-shadow cursor-pointer"
              >
        ...
        <div>
          ...
          {selectedStaffId && !loading && records.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-2.5 py-14 px-6">
              ...
              <p className="text-[13px] font-medium text-gray-700">No extra shifts</p>
              <p className="text-[12px] text-gray-400 text-center leading-relaxed">
                {selectedStaffId === "all"
                  ? "No team members have any extra shifts set."
                  : `${selectedStaff?.nickname} has no extra shifts. Use the button above to add one.`}
              </p>
            </div>
          )}
          ...
          {selectedStaffId && !loading && records.length > 0 && (
                    ...
                    >
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[12px] ${past ? "text-amber-600" : "text-green-600"}`}>
                            {date.format("dddd")}
                          </span>
                          {past && (
            ...
            <div className="flex items-center justify-between px-6 py-3 border-t border-gray-100">
              <select
                ...
                className="appearance-none bg-white border border-gray-200 rounded-md px-3 py-1.5 pr-7 text-[13px] text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500 cursor-pointer"
              >
                {[10, 25, 50].map((size) => (
        ...
        <DialogContent>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5, mt: 1 }}>
            ...
            <TextField
              ...
              placeholder="e.g. Covering for colleague"
            />
            {formError && (
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ color: "text.secondary", lineHeight: 1.5 }}>
            This staff member will no longer be available on that date unless it falls in their regular working days.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button
...
export default ManageStaffExtraWorkingDayPage;
